import { createUser, deleteUsers, findUserToBeDeleted, getAllRoles, getDirectusUserByEmail, getUser, getUserByEmail } from "../daos/users";
import { ACCOUNTABILITY } from "../utils/config";

// Function to synchronize user data
const syncUser = async (email: string, schema: any, { services, database }: any) => {
    const { ItemsService, UsersService } = services;

    // Check if email is provided
    if (email) {
        const rolesService: any = new ItemsService('directus_roles', { ACCOUNTABILITY, schema });
        ...
        const directusUserQuery = await getDirectusUserByEmail(email, directusUsersService);
        ...
        if (directusUser?.id) {
            ...
            if (existingUser?.organization_id && existingUser?.organization_id !== directusUser?.organization_id) {
...
const userDelete = async (keys: any[], _: any, database: any, UsersService: any, ItemsService: any, schema: any) => {
    if (keys?.[0]) {
        ...
        const toDelete: any[] = []
        await Promise.all(keys.map(async (userId: string) => {
            const userQuery = await getUser(userId, userItems)

            if (userQuery?.[0]?.email) {
                const directusUserQuery = await findUserToBeDeleted(userQuery, directusUsersService)
...
const userUpdate = async (keys: any[], ItemsService: any, schema: any, services: any, database: any) => {
    if (keys?.[0]) {
