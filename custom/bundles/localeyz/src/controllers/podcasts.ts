...
import { createPodcastEpisodes, deletePodcastEpisodes, getPodcastEpisodes, getPodcasts, updatePodcast, updatePodcastImage } from '../daos/podcasts';
...
const handler = async ({ url, keys, user, token, database }: { url: string | null | undefined, keys: string, user?: string | null, token: string, database: any }) => {
    if (!url) {
        return;
    }
...
const run = async (podcastId: string, rssFeed: string, context: { database: any }, req: any, res: any) => {
    const { database } = context;
    let accountability: any;
    let podcast: {
        skipRecursion?: boolean, title: string, image: string, description: string, directus_image: string
    } = {
        title: '',
        image: '',
        description: '',
...
const podcaseScheduler = async (req: any, res: any, context: any) => {
    ...
    for (let i = 0; i < podcasts.length; i++) {
        if (!podcasts[i].directus_image)
            await run(podcasts[i]?.id, podcasts[i]?.rss_feed, {
                database
            }, req, res);
