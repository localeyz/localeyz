...
const globalImage = async (req: any, res: any, keys: any, context: any) => {
    try {
        ...
        const image = keys?.payload?.image || keys?.payload?.image_url || keys?.payload?.thumbnail_url;
        const collections = ['groups', 'live_video_episodes', 'live_videos', 'organizations', 'podcast_episodes', 'producer_portal_widgets', 'radios', 'programs', 'episodes']
...
const globalImageScheduler = async (req: any, res: any, context: any) => {
    try {
        ...
        const collections = ['groups', 'organizations', 'podcast_episodes', 'producer_portal_widgets', 'radios', 'programs', 'episodes', 'live_videos', 'live_video_episodes']
...
const handler = async ({ url, user, token }: { url: string | null | undefined, user?: string | null, token: string }) => {
    if (!url) {
        return;
    }
...
const processCollection = async (collection: any, service: any) => {
    if (collection === 'episodes') {
        ...
        await chunkProcess((collection === 'producer_portal_widgets') ? "image_url" : "image", imageFieldName, service);
    }
}
...
const chunkProcess = async (urlField: string, imageField: string, service: any, chunkSize: number = 10000) => {
    let offset = 0;
    let batch;
        ...
        await Promise.all(batch.map(async (item: any) => {
            ...
            if (isValidURL(url)) {
                ...
                if (imageId) {
                    ...
                    await uploadImage(item.id, imageId, (urlField === 'thumbnail_url') ? 'thumbnail' : 'directus_image', service);
                }
            }
        }));
