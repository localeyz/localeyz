import { createActivity, createSession, createUser, getAllRoles, getUserByEmail, updateUser } from "../daos/sso";
...
const userLogin = async (req: UserLoginRequest, res: UserLoginResponse, { services, database, getSchema }: any) => {
    ...
    const expiresDateTime = new Date(Date.now() + 2.628e+9);
    ...
    try {
        ...
        if (role) {
            foundRole = roles.find((r: { name: string }) => r.name.toLowerCase() === role);  // Find role by name
        }

        if (!user) {
            const userId = createUser(req, email, id, foundRole, organization, usersService);  // Create new user if not found
    ...
    try {
        ...
        await createSession(sessionService, accessToken, user, expiresDateTime, accountability);
